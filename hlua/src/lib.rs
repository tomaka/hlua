...
impl<'lua> Lua<'lua> {
    ...
    pub fn open_coroutine(&mut self) {
        unsafe {
            ffi::luaopen_coroutine(self.lua.0);
        }
    }
    ...
    pub fn open_package(&mut self) {
        unsafe {
            ffi::luaopen_package(self.lua.0);
        }
    }
